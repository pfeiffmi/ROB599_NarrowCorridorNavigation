...
class MainApp(QT_widgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        self.lidar_view.setFixedWidth(math.ceil(0.5*self.full_screen_width))
    ...
    # Define the slot to set the image to the label
    @QT_core.pyqtSlot()
    def tactile_thread_start(self):
        # Log a message thaat the tactile thread has started
        rclpy.logging.get_logger("tact-slot").info("Tactile thread started!")
...
def main():
    ...
    window = MainApp(display_update_ms=2000) 
    sys.exit(app.exec())
